export function buildStoryline(output) {
    ...
    if (output.outcomeRisk === "safe")
        headline = "Zero Baggage Fees Detected";
    if (output.outcomeRisk === "high_cost")
        headline = "High Baggage Penalty Warning";
    let verdict = `Your estimated total baggage cost is ${totalBaggageCost} ${currency}.`;
    if (totalBaggageCost === 0)
        verdict = "You have no estimated baggage fees for this configuration.";
    let costSummary = `Base fees: ${baseBaggageFeeTotal} ${currency}. `;
    if (overweightFeeTotal > 0)
        costSummary += `Overweight penalties: ${overweightFeeTotal} ${currency}. `;
    if (oversizeFeeTotal > 0)
        costSummary += `Oversize penalties: ${oversizeFeeTotal} ${currency}. `;
