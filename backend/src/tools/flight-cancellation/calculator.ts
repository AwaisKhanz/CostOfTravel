import type { FlightCancellationInput, FlightCancellationOutput, Scenario, FarePolicy } from './types.js';

export function calculateOutput(
  input: FlightCancellationInput,
  scenario: Scenario,
  policy: FarePolicy | null
): FlightCancellationOutput {
  const round = (val: number) => Math.round(val * 100) / 100;
  
  const baseOutput: FlightCancellationOutput = {
    schemaVersion: "v1",
    scenario,
    refundAmount: 0,
    creditAmount: 0,
