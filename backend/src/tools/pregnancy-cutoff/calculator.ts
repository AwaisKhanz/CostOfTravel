import type { PregnancyTravelInput, PregnancyTravelOutput, Scenario } from './types.js';
import type { MapperOutput } from './scenario.mapper.js';

export function calculateOutput(input: PregnancyTravelInput, mapper: MapperOutput): PregnancyTravelOutput {
  const { scenario } = mapper;

  const baseOutput: PregnancyTravelOutput = {
    schemaVersion: "v1",
    scenario,
