import type { NoShowPolicy } from './types.js';

const noShowPolicies: Record<string, Record<string, NoShowPolicy>> = {
  delta: {
    basic_economy: {
...
export function fetchNoShowPolicy(airline: string, fareClassId: string): NoShowPolicy | null {
  const airlineKey = airline.toLowerCase();
  if (!noShowPolicies[airlineKey]) return null;
