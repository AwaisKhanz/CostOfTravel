import type { FamilyBaggageInput, FamilyBaggageOutput, BaggagePolicy, Scenario } from "./types.js";
...
export function calculateBaggageFee(
  ...
  let outcomeRisk: FamilyBaggageOutput["outcomeRisk"] = "partial_loss";
  if (totalBaggageCost === 0) outcomeRisk = "safe";
  else if (overweightFeeTotal > 0 || oversizeFeeTotal > 0) outcomeRisk = "high_cost";

  let reasonCode: FamilyBaggageOutput["reasonCode"] = "STANDARD_BAGGAGE_FEES";
  if (input.numberOfPassengers > 1) reasonCode = "FAMILY_MULTIPLIED";
  if (overweightFeeTotal > 0) reasonCode = "OVERWEIGHT_STACKED";
  if (oversizeFeeTotal > 0) reasonCode = "OVERSIZE_STACKED";
