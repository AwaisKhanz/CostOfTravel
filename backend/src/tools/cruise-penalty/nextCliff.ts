import type { CruisePolicy } from './policy.fetch.js';

export interface NextCliffInsight {
  nextPenaltyCliffDays: number;
  nextPenaltyCliffPercent: number;
  daysUntilNextCliff: number;
}

export function getNextCliff(
  policy: CruisePolicy,
  currentMatchedMinDays: number | null,
  daysBeforeSail: number,
): NextCliffInsight | null {
  if (currentMatchedMinDays === null) return null;
