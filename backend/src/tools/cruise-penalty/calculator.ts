import type { CruisePenaltyInput, CruisePenaltyOutput } from './types.js';
import type { MapperOutput } from './scenario.mapper.js';
...
export function calculateOutput(
  ...
  const penaltyAmount = roundToTwoDecimals(
    input.totalTripCost * (finalPenaltyPercent / 100)
  );
  
  const refundAmount = roundToTwoDecimals(
    input.totalTripCost - penaltyAmount
  );
