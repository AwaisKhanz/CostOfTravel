import type { BudgetCheckinInput, BudgetCheckinOutput } from './types.js';
...
export function calculateOutput(
  ...
  const outcomeRisk = 
    scenario === "checkin.already_checked_in" || scenario === "checkin.online_still_available" 
      ? "safe" : "fee_applies";

  const reasonCode = 
    scenario === "checkin.already_checked_in" ? "ALREADY_CHECKED_IN" :
