...
export default function RefundVsCreditQuizPage() {
  ...
  const submitQuiz = async () => {
    ...
    const payload = {
      ...
      isAirlineInitiated: answers.isAirlineInitiated === "true" || answers.isAirlineInitiated === true,
    ...
    return (
      <div className="bg-card p-8 rounded-card border border-border-subtle shadow-premium animate-in fade-in slide-in-from-bottom-4">
        ...
        <div className="mt-10 flex gap-4">
          ...
          >
            {step === quizQuestions.length - 1 ? 'Analyze Payout' : 'Continue'}
      ...
      <div className="max-w-xl mx-auto py-12">
        {isLoading && step === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
             ...
             <p className="text-foreground/40 font-bold animate-pulse uppercase tracking-widest text-xs">Fetching Fare Policy...</p>
          </div>
        ) : (
          <>
            {step < 99 && (
            ...
            {step === 99 && calculating ? (
               <div className="bg-card p-12 rounded-card border border-border-subtle shadow-premium text-center">
                  <Banknote className="h-16 w-16 text-brand-primary mx-auto mb-6 animate-bounce" />
                  <h2 className="text-2xl font-extrabold text-foreground tracking-tight animate-pulse mb-2">Auditing Refund Entitlement...</h2>
                  <p className="text-sm text-foreground/40 font-medium font-mono">Reconcilling carrier rules with consumer laws...</p>
               </div>
            ) : result ? (
              <div className="animate-in zoom-in-95 duration-500">
                ...
                <div className="mt-8 text-center">
                   ...
                   >
                     <ArrowLeft className="h-4 w-4" /> Start New Audit
                   </button>
                </div>
              </div>
            ) : (
